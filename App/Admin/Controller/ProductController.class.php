...
class ProductController extends PublicController{
	...
	public function index(){
		...
		foreach ($productlist as $k => $v) {
			...
			$productlist[$k]['is_activity'] = M('activity')->where('id='.intval($v['is_activity']))->getField('name');
			$productlist[$k]['city_id'] = M('city')->where('id='.intval($v['city_id']))->getField('name');
	...
	public function add(){	
		...
		try{	
			...
			$array=array(
				...
				'city_id'=>intval($_POST['city_id']),//是否折扣
				'is_activity'=>intval($_POST['is_activity']),//参与活动类型
			);
		...
		// 查询所有产品分类
		//=========================
		$cate_list = M('category')->where('bz_2 = 1')->field('id,name')->select();
		$this->assign('cate_list',$cate_list);
         //查询商户信息
         $business = M('businessuser')->where(['del'=>0])->field('id,name')->select();
		//=========================
		// 查询产品信息
		//=========================
