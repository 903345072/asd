...
class ProductController extends PublicController {
    ...
    public function index(){
		...
		// 2019年2月20日21:06:32
		$pro['is_activity'] = M('activity')->where('id='.intval($pro['is_activity']))->getField('name');
	...
	//***************************
   	public function activity_lists(){
 		$json="";
		$is_activity = intval($_POST['is_activity']);
		...
		if($is_activity == false){
			$flag = true;
			$activity_list = M('activity')->where('is_status = 1')->order('sort asc')->field('id,name')->select();	
			$is_activity = $activity_list[0]['id'];
		}

 		if (intval($is_activity)) {
 			$where.=" AND is_activity=".intval($is_activity);
 		}
 		if($keyword) {
		...
		echo json_encode(array('status'=>1,'pro'=>$json_arr,'list'=>$activity_list));
 		exit();
    }
	//***************************
	//  获取商品列表接口
	//***************************
   	public function lists(){
		...
		$cat_name=M('category')->where("id=".intval($id))->getField('name');
		echo json_encode(array('status'=>1,'pro'=>$json_arr,'class_list'=>$class_list,'cat_name'=>$cat_name));
